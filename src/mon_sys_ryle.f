...
C         Main Code
  ...
  100 continue
          ...
          else if  (command .eq. check_LOalc_cmd)  then
              call check_alcs_ryle (def_sf, plot_device, s) 
          else if  (command .eq. check_IFalc_cmd)  then                
              call check_IFalcs_ryle (def_sf, plot_device, s) 
...
c             call save_samp_ryle (def_sf, s) 
          ...
          else if  (command .eq. shadow_cmd)  then
              call flag_shad_ryle (def_sf, def_lsf, s) 
          else
