...
C+ ENQ_AE_VIS
       ...
       subroutine enq_ae_vis ( lun, n, iae1, iae2, s )
C
C return the aerials for the n'th visibility in the sample file
C
C Input
...
C call appropriate enquiry routine version
       if (ct_vers .le. 1) then
         call enq_v1_ae_vis( n, iae1, iae2, s )
       elseif (ct_vers .eq. 2) then
         call enq_v2_ae_vis( n, iae1, iae2, s )
       else
         s = ILL_CONTTAB
         goto 999
       endif
 ...
 999   call smp_wrerr( s, 'in subroutine ENQ_AE_VIS' )
