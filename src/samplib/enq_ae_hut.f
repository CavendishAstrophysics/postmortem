C+ ENQ_AE_HUT
       ...
       subroutine enq_ae_hut ( lun, n, iae1, iae2, s )
C
C return the first and last aerials for the n'th hut in the sample file
C
C Input
...
C   S             -     I4     -   Error return
C
C Computes the index numbers of the first and last aerials in hut N.
C If N does not correspond to a valid hut the indexes are set to 0.
C If the current telescope type is set to 5-km then ILL_TSCOPE is
C returned as status.
C
C [PA, 4/11/88]
*-
...
C call appropriate enquiry routine version
 ...
 999   call smp_wrerr( s, 'in subroutine ENQ_AE_HUT' )
