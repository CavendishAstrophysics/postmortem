...
*+PLOT_PCS_CLFST
       ...
       subroutine plot_pcs_clfst (ifile, list, ilist, nae,
     :                          isamp1, isamp2, plot_device, status)
C      -------------------------------------------------------------
C
C  Plots PC values by aerial.
C
C  Given:
...
C
C  Subroutine to produce plots of the PC settings recorded with the sample
...
c
         ...
         do isamp=isamp1,isamp2,istep
           ...
           if (status.eq.0) then
             ...
             do i=1,mae
               iae=ilist(ipass+i-1)
               call enq_mon_pc( ifile, iae, ipc, status )
               yy(n,i)=ipc
             enddo
           endif
         enddo
c
c    Produce plot for each aerial in turn
c
         do i=1,mae
           iae=ilist(ipass+i-1)
...
c             Plot graph
               ...
               call pglabel( 'Sample number', 'PC values', ' ' )
c
c             Plot title
