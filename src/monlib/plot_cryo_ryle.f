*+PLOT_CRYO_RYLE
       ...
       subroutine plot_cryo_ryle (ifile, list, ilist, nae,
     :                 isamp1, isamp2, max_value, plot_device, status)
C      ---------------------------------------------------------------
C
C  Plots cryo temperatures by aerial.
C
C  Given:
...
C
C  Subroutine to produce plots of the cryo temperature readings recorded
...
c
       text='Ae xx,  Cryo temperature readings'
...
c
       ...
       do isamp=isamp1,isamp2,istep
         ...
         if (status.eq.0) then
           ...
           do i=1,nae
             iae=ilist(i)
             call enq_mon_cryo( ifile, iae, yy(n,i), status )
           enddo
         endif
       enddo
c
...
c         Plot graph
           ...
           call pglabel( 'ST (hrs)', 'Cryo temperature', ' ' )
c
c         Plot title
