*+CHECK_CRYO_RYLE
       ...
       subroutine check_cryo_ryle (file, plot_device, status)
C      ------------------------------------------------------
C
C  Executes the CHECK-CRYO-TEMP command.
C
C  Given:
...
C
C  This command scans the sample file and reports the cryo temperature
C  readings recorded during the observation, for selected aerials.
C  A table is produced of minimum, maximum and mean readings (and rms
C  deviations) during the run.
C
...
C 11 Aug 03  GP:  default answer changed
*-
...
c          do i=1,nae
c            call enq_mon_cryo(ifile,i,value,status)
...
c
         if (option.eq.'SCAN') then
           ...
           if (status.eq.0) then
             if (iout.ne.termo) write(iout,*)'CHECK-CRYO-TEMP'
             write(iout,*)
...
c
       if (option.eq.'SCAN') then
         call scan_cryo_ryle(ifile,isamp1,isamp2,log,rmax,status)
c
c  Plot readings for selected aerials
c
       elseif (option.eq.'PLOT') then
         call plot_cryo_ryle(ifile,list,ilist,nae,isamp1,isamp2,
     :                                     max_value,plot_device,status)
c
c  Print readings for selected samples
c
       elseif (option.eq.'SHOW') then
         call show_cryo_ryle(ifile,status)
       endif
c
       if (status.eq.USR_BREAK) then
         status=0
       else if (status.ne.0) then
         call mon_wrerr(status,'in routine CHECK_CRYO_RYLE')
       endif
c
