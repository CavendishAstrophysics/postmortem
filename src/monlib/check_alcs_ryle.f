*+CHECK_ALCS_RYLE
       ...
       subroutine check_alcs_ryle (file, plot_device, status)
C      ------------------------------------------------------
C
C  Executes the CHECK-ALCS command.
C
C  Given:
...
c
         if (option.eq.'SCAN') then
           ...
           if (status.eq.0) then
             if (iout.ne.termo) write(iout,*)'CHECK-ALCS'
             write(iout,*)
...
c
       if (option.eq.'SCAN') then
         call scan_alcs_ryle(ifile,isamp1,isamp2,log,rmax,status)
c
c  Plot ALC values for selected aerials
c
       elseif (option.eq.'PLOT') then
         call plot_alcs_ryle(ifile,list,ilist,nae,isamp1,isamp2,
     :                                     max_value,plot_device,status)
c
c  Print ALC values for selected samples
c
       elseif (option.eq.'SHOW') then
         call show_alcs_ryle(ifile,status)
       endif
c
       if (status.eq.USR_BREAK) then
         status=0
       else if (status.ne.0) then
         call mon_wrerr(status,'in routine CHECK_ALCS_RYLE')
       endif
c
