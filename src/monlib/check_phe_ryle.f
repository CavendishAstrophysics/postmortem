*+CHECK_PHE_RYLE
       ...
       subroutine check_phe_ryle (file, plot_device, status)
C      -----------------------------------------------------
C
C  Executes the CHECK-HELIUM command.
C
C  Given:
...
C
C  This command scans the sample file and reports Helium pressure readings
...
c          do i=1,nae
c            call enq_mon_phe(ifile,i,value,status)
...
c
         if (option.eq.'SCAN') then
           ...
           if (status.eq.0) then
             if (iout.ne.termo) write(iout,*)'CHECK-HELIUM'
             write(iout,*)
...
c
       if (option.eq.'SCAN') then
         call scan_phe_ryle(ifile,isamp1,isamp2,log,rmax,status)
c
c  Plot readings for selected aerials
c
       elseif (option.eq.'PLOT') then
         call plot_phe_ryle(ifile,list,ilist,nae,isamp1,isamp2,
     :                                     max_value,plot_device,status)
c
c  Print readings for selected samples
c
       elseif (option.eq.'SHOW') then
         call show_phe_ryle(ifile,status)
       endif
c
       if (status.eq.USR_BREAK) then
         status=0
       else if (status.ne.0) then
         call mon_wrerr(status,'in routine CHECK_PHE_RYLE')
       endif
c
