*+PLOT_VAC_RYLE
       ...
       subroutine plot_vac_ryle (ifile, list, ilist, nae,
     :                 isamp1, isamp2, max_value, plot_device, status)
C      ---------------------------------------------------------------
C
C  Plots vacuum readings by aerial.
C
C  Given:
...
C
C  Subroutine to produce plots of the vacuum readings recorded with the
C  sample file currently opened on logical unit IFILE, for each aerial
...
c
       ...
       do isamp=isamp1,isamp2,istep
         ...
         if (status.eq.0) then
           ...
           do i=1,nae
             iae=ilist(i)
             call enq_mon_vac( ifile, iae, yy(n,i), status )
           enddo
         endif
       enddo
c
...
c         Plot graph
           ...
           call pglabel( 'ST (hrs)', 'Vacuum readings', ' ' )
c
c         Plot title
