...
C     Initialise aperture and beam aperture arrays.
  ...
  100 continue
      ...
      do 700, i = 1, numlsf
          ...
          string(28:47) = '>-------------------'
c         if (exe_mode.eq.0) then
c           call io_wrout( string )
c         else
...
C             Add in sources.
  ...
  300         continue
          ...
          do 600, j = 1, num_buff
c             if ( mod(j, max0(1,int(num_buff/20))) .eq. 0 ) then
c                 ls = chr_lend( string, '>' )
c                 string(ls+1:ls+2) = '->'
c                 if (exe_mode.eq.0) call io_wrout( string )
c             end if
     ...
     *                         num_vis,
     *                         s          )
c             call user_action(num_vis, vis,
c    *                         lsf_num, j, num_buff, s )
              if ( s .ne. 0 ) goto 9999
