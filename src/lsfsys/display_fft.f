...
C+display_fft
C
      SUBROUTINE display_fft(  lsf_num,
     *                         plot_device,
     *                         s                      )
...
C         Status variable - must be zero on entry - otherwise error
              integer             s
C
C     Displays ffts of logical sample file buffers on the plot
C     device.
C
C-
C=======================================================================
...
C         Spacing list and string description.
              ...
              character*(80)  list
C         Radii of each spacing and maximum radius.
              real            radii( max_vis )
              real            max_radius
...
C             and initialise the index array
              ...
              do 500, i = 1, num_pts
                  ...
                  min_plot = min0( plot_list( i ), min_plot )
...
C                 Plot graph
                  ...
                  call pgrnge( min_plot, max_plot, min_plot, max_plot )
                  tick_intvl = pgrnd((max_plot/2.5), ntick )
     ...
     *                          'FFT of instantaneous visibility' )
              ...
              write(*,'(X,A,I5)') 'No data in buffer ', buff_num
          end if
...
C
 9999 continue
          if ( s .ne. usr_break ) then
              call lsf_wrerr( s, 'in subroutine DISPLAY_FFT' )
          end if
          return
      end
