*+ flag_err
       ...
       subroutine flag_err( istat, routine, message )
C      ----------------------------------------------
C
...
C
C P. Alexander MRAO, Cambridge.  13/08/91
*-
...
C local buffer signalling error file being defined
       integer                set_flag_err_file
       common /flag_err_save/ set_flag_err_file

C define error file name
...
C set up local error messages if not done so already
       if (set_flag_err_file.ne.-1) then
         s = 0
         call io_setmsg( flag_error_file, s)
         set_flag_err_file = -1
       end if
