...
C+lsf_sel_notspac
C
      SUBROUTINE lsf_sel_notspac( lsf_num,
     *                            s                      )

C
C     Asks the user to select logical sample file spacings to exclude.
C
C     Given:
...
C
      ...
      call get_spacings( sf_lun, 'Spacings list to exclude : ', 'All',
     *                   description, spacings, max_vis, num_spac, s )
      call lsf_set_spacings( lsf_num, num_spac, spacings, 4, s )
      if ( s .ne. 0 ) goto 9999
      return
...
C
 9999 continue
          if ( s .ne. usr_break ) then
              call lsf_wrerr( s, 'in subroutine LSF_SEL_NOTSPAC' )
          end if
          return
      end
