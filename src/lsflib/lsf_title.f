...
C         Modified Julian date for zero of sidereal time
              real*8          mjd0
C         UT at zero of sidereal time, sample sidereal time (hours)
              real*8          ut0, st
C         Sample UT (seconds)
              integer*4       samp_ut
...
C
     ...
     *    name(1:ls), '  centred on  ', source_text
      ...
      write( string, '(A,I3,2(''.'',I2.2))' ) '   ST:',
     *                                      itime(3), itime(2), itime(1)
      ls = chr_lenb( string )
      lt = chr_lenb( title(3) )
      title(3)(lt+1:lt+ls+1) = string(1:ls)
      call enq_mjd_st0( sf_lun, mjd0, s )
      ut0 = (mjd0 - int(mjd0))*24.D0
      st = dfloat(samp_sid/10)/3600.D0
      samp_ut = int( dmod((ut0+st/const_sut2sst),24.D0) *3600.D0 )
      call util_stohms(samp_ut,itime)
      write( string, '(A,I3,''.'',I2.2)' ) '   UT:', itime(3), itime(2)
      ls = chr_lenb( string )
      lt = chr_lenb( title(3) )
      title(3)(lt+1:lt+ls+1) = string(1:ls)
      write( string, '(A, I4)' ) '   LSF buffer: ', first_buffer
      ls = chr_lenb( string )
      lt = chr_lenb( title(3) )
      title(3)(lt+1:lt+ls+1) = string(1:ls)
     ...
     *                                      itime(3), itime(2), itime(1)
          ...
          samp_ut = int( dmod((ut0+st/const_sut2sst),24.D0) *3600.D0 )
          call util_stohms(samp_ut,itime)
          write( string, '(A,I3,''.'',I2.2)' ) '   UT:',
     *                                      itime(3), itime(2)
          ls = chr_lenb( string )
          lt = chr_lenb( title(4) )
          title(4)(lt+1:lt+ls+1) = string(1:ls)
          write(string, '(A, I4)' ) '   LSF buffer: ', last_buffer
          ls = chr_lenb( string )
          lt = chr_lenb( title(4) )
          title(4)(lt+1:lt+ls+1) = string(1:ls)
      else
