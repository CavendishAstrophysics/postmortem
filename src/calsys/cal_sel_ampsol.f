...
C+cal_sel_ampsol
C
      SUBROUTINE cal_sel_ampsol( s )

C     Asks the user to select whether amplitude is solved for
C
C     Given:
C         None.
C
...
C
      ...
      if (io_onoff('Amplitude-calibration :  ',default,s)) then
        cal_no_amp = .false.
      else
        cal_no_amp = .true.
      end if
...
C
 9999 continue
          if (s .ne. USR_BREAK) then
              call cal_wrerr( s, 'in subroutine cal_sel_ampsol ' )
          end if
