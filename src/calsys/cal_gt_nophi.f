...
C+cal_gt_nophi
       ...
       subroutine cal_gt_nophi( s )
C      ----------------------------
C
C Reset the current gains table so the phases are zero
C
C      Returned:
C         error status
                  integer      s
C
C Reset the current gains table so that the phase correction factors
C are set to zero.
C
C PA 24/05/90
C-
...
C    counters
     ...
     *          cmplx(cabs(ae_gains(ich,isb,iae)),0.0)
       ...
       if (s.ne.0) call cal_wrerr(s,'in cal_gt_nophi')
