...
! ^request 'flag out 2/3B?'                      '<flag23B>' flag23B
! ^request 'flag out 5C?'                        '<flag5C>' flag5C
! ^request 'flag out 5D?'                        '<flag5D>' flag5D
! ^request 'flag out 6C?'                        '<flag6c>' flag6c
...
! ^system /usr/bin/rm <sd>/<calsource>-<date>/cal
! ^system /usr/bin/rm <sd>/<calsource>-<date>/lsf
...
^if  %match (<flag23B>, No) ^jump F23B1
lsf
flag add 0
ae 2/3; sb B
1-3000
...
^request 'name of target source : '            '<target>' target
...
^if  %match (<flag23B>, No) ^jump F23B2
lsf
flag add 0
ae 2/3; sb B
1-3000
...
cc now make calibration for <target> ...
calibration
solution check-closure 10
cal-file
yes yes
...
^request 'last  sample :' '4000', lastsamp
...
^loop-marker NEWTAR
^if %match (<target>, CYGNUSX1) ^assign target CYGNUSX3
^jump MAIN
